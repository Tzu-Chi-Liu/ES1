...
def diagnostics_animation(t,NSP,N,r,v,NG,x_grid,rho_grid,phi_grid,E_grid,step,units,pause_time,save_dir):
    ...
    # phase space in upper left hand corner
    ...
    # velocity distribution in upper right hand corner
    ...
    # density at grid positions in lower left hand corner
    ...
    # potential and field at grid positions in lower right hand corner
    ax4=animation_fig.add_subplot(224)
    ax5=ax4.twinx()
    ax4.plot(x_grid,phi_grid,'blue')
    ax5.plot(x_grid,E_grid,'orange')
    ...
    ax5.set_ylabel('electric field E '+units['E_grid'],color='orange')
    ax4.set_title('Potential and Electric Fields')
    ax4.set_ylim(-1.1*np.max(np.abs(phi_grid)),1.1*np.max(np.abs(phi_grid)))
    ax5.set_ylim(-1.1*np.max(np.abs(E_grid)),1.1*np.max(np.abs(E_grid)))
    # ax4.set_ylim(-1.5e-8,1.5e-8)
    # ax5.set_ylim(-1e-7,1e-7)
